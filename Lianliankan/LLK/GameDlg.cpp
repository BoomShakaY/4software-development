...
void CGameDlg::OnBnClickedBtnStart()
{
	...
	//初始化进度条
	m_GameProgress.SetRange(0,60*5);  //设置范围
	m_GameProgress.SetStep(-1);         //初始步数值
	m_GameProgress.SetPos(60*5-1);        //设置初始值
...
// 	DrawGameTime();  //直接在消息相应里写了
	...
	m_Mytip.AddTool( GetDlgItem(IDC_BUTTON3), _T("你还有5次使用提示的机会"));
	this->GetDlgItem(IDC_BUTTON3)->SetWindowTextW(_T("提示(5)"));
	//刷新
	...
	//查找联通路径
	 ...
	 //如果能够联通，绘制提示框和连线并更新游戏地图
       ...
       //更新文字和提示框	
	    ...
	    m_Mytip.AddTool( GetDlgItem(IDC_BUTTON3), _T("你还有"+str+_T("次使用提示的机会")));
	    this->GetDlgItem(IDC_BUTTON3)->SetWindowTextW(_T("提示(")+str+_T(")"));
...
void CGameDlg::OnClickedBtnStop()
{
	...
	if (m_bPause == false)
	{
		this->GetDlgItem(IDC_BUTTON2)->SetWindowTextW(_T("恢复游戏"));
	...
	{
		this->GetDlgItem(IDC_BUTTON2)->SetWindowTextW(_T("暂停游戏"));
		UpdateMap();
		InvalidateRect(m_rtGameRect,FALSE);
